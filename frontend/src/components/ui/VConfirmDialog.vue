<!--
  ...
  Optional `title` (Master GROUPS P3, PROMPT №592): a bold heading line above
  ...
  G6/G8 (PROMPT №609): `title`'s font-size stepped down from --text-xl (32px)
  ...
  Optional `compactActions` (PROMPT №609, G10): renders both action buttons
  ...
  Default SLOT (PROMPT №610, owner Q9): optional content rendered between
  ...
  Optional `warningPanel` (PROMPT №610, owner Q9): renders `message` inside
  ...
  Optional `warningPanelIcon` (T24-15, PROMPT №634): the block-confirm and
  ...
  Optional `cancelVariant` (T24-14/16/31, PROMPT №634): the owner's position-
  ...
  Optional `titleStrong` (T24-28, PROMPT №634): a prop, not a caller-side
...
withDefaults(
  defineProps<{
    open: boolean
    /** Optional bold heading above `message` (P3, PROMPT №592). Omit for the
...
/* Mirrors VBottomSheet's .v-sheet__title exactly -- same heading token
   ...
   --text-lg (PROMPT №609, G6/G8) so a two-word title question fits one
   line -- was --text-xl. */
.v-confirm__title {
...
/* T24-28 (PROMPT №634) -- see the file header for why this is a prop-driven
...
/* warningPanel (PROMPT №610, owner Q9): SAME peach recipe as
...
.v-confirm__panel {
  display: flex;
  /* T24-13 (PROMPT №634): icon centred against the paragraph, not pinned to
...
.v-confirm__panel-text {
  ...
  color: var(--velo-peach-500);
  text-align: left;
  /* T24-17/T24-30 (PROMPT №634): measured from the owner's SVG -- a 17px
...
.v-confirm__actions {
  ...
  /* Wrap only when the buttons' real content width doesn't fit the row (bug
     4, PROMPT №408) -- a global flex-direction: column would restack every
