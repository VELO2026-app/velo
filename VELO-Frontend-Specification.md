...
- [x] **Опер-гейт seed-мегапака (№255)** — bare `velo seed-practices` требует typed-«yes» перед сценарием `default` (~300 практик/12 мастеров); `--scenario`/`--yes`/`--dry-run` обходят; reset+default = один объединённый prompt. ⚠ **B36, PROMPT №724: `seed-practices` больше не существует** — сид-консолидация (`3184616d`/`8b7aa1a4`) удалила `seed_practices.py`/`seed_demo.py`; единственная живая команда — `velo seed [--profile NAME] [--reset] [--list]`. Запись выше — исторический факт того, что было построено на №255, не инструкция к запуску сегодня.
